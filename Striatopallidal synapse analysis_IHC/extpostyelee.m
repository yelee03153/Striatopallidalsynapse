...
function res_clustering_post = extpostyelee (input_post,params)
...
%% Denoising
res_denoising = imgaussfilt(input_post,params.deno3);
...
res_idx = find(res_tophat> params.hard_lev3);
...
res_bw = imbinarize(res_thres, check_threshold* params.otsu3);

% figure, imagesc(res_bw), colormap gray, title('thresholding')

res_bw2 = bwareaopen(res_bw, params.deno_px_number3);
...
% 
% %%
% res_clustering_post = zeros(size(res_label_post));
% [~,n22] = size(new_label);
% 
% for ni= 1:n22
...
%         idx4 =  find(res_label_post==temp_idx(qi));
% 
%         res_clustering_post(idx4) = ni;
%     end
%     
% end
