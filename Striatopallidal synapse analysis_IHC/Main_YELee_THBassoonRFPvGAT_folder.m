...
folder_name = "E:\2) A2a-CreAi9_TH Bassoon RFP GABAaR X63 X3 stitching"; 
image_list = extractFileList(folder_name); 


variable_list = {'colocarea1', 'prenum1', 'postcolocarea1', 'postnum1', 'synapsearea1', 'synapsenum1', 'THarea1', 'THnum1', 'rfparea1','colocarea2', 'prenum2', 'postcolocarea2', 'postnum2', 'synapsearea2', 'synapsenum2', 'THarea2', 'THnum2', 'rfparea2','colocarea3', 'prenum3', 'postcolocarea3', 'postnum3', 'synapsearea3', 'synapsenum3', 'THarea3', 'THnum3', 'rfparea3'};
variable_list2 = {'min_dmat1,2,3'}
...
if v == 2
    colocarea2 = nnz(res_clustering_pre);
end

if v == 3
    colocarea3 = nnz(res_clustering_pre);
end

res_clustering_TH = extTH(input_body, params);
if v == 1
   THarea1 = nnz(res_clustering_TH);
   THnum1 = max(max(res_clustering_TH));
   prenum1 = max(max(res_clustering_pre));
end
if v == 2
   THarea2 = nnz(res_clustering_TH);
   THnum2 = max(max(res_clustering_TH));
   prenum2 = max(max(res_clustering_pre));
end
if v == 3
   THarea3 = nnz(res_clustering_TH);
   THnum3 = max(max(res_clustering_TH));
   prenum3 = max(max(res_clustering_pre));
end
...
if v == 1
    postcolocarea1 = nnz(res_clustering_post);
    postnum1 = max(max(res_clustering_post));
end
if v == 2
    postcolocarea2 = nnz(res_clustering_post);
    postnum2 = max(max(res_clustering_post));
end
if v == 3
    postcolocarea3 = nnz(res_clustering_post);
    postnum3 = max(max(res_clustering_post));
end
...
if v == 2
    rfparea2 = nnz(res_clustering_rfp);
end
if v == 3
    rfparea3 = nnz(res_clustering_rfp);
end
...
for f = 1:NPpost
        min_dmatPost(f,1) = min(dmat_prepost1(f,:));
end
    
NPpre = max(max(res_clustering_pre));

for g = 1:NPpre
        min_dmatPre(g,1) = min(dmat_prepost1(:,g));
end
...
writematrix(min_dmat1,filename,'Sheet',2,'Range',output_matrix(iteration+1)+string(3));
writematrix(min_dmatPost,filename,'Sheet',3,'Range',output_matrix(iteration+1)+string(3));
writematrix(min_dmatPre,filename,'Sheet',4,'Range',output_matrix(iteration+1)+string(3));
