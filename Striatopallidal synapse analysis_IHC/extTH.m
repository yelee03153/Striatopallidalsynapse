...
function THarea = extTH (input_body,params)
...
%% Denoising
res_denoising = imgaussfilt(input_body,params.deno2);
...
res_idx = find(res_tophat> params.hard_lev2);
...
res_bw = imbinarize(res_thres, check_threshold* params.otsu2);

% figure, imagesc(res_bw), colormap gray, title('thresholding')

res_bw2 = bwareaopen(res_bw, params.deno_px_number2);
%  figure, imagesc(res_bw2), colormap gray, title('noise suppresion')



%% Opening 
%  assign lower limit(pixel length): measure the area in ImageJ, convert to
% pixel unit, in a root square to measure the length
% Under 10X10 pixel will be not considered as a TH signal(in synaptophysin
% virus protocol. other protocol we used 1)
% opt2 = strel('sphere',5);
% 
...
res_label_TH = bwlabeln(res_fill);
THarea = res_label_TH;
% 
