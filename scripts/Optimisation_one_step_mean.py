...
def synaptic_response(params, condition,scaling_params = None):
    ...
    if idx1.size > 0 and idx2.size > 0:
        ...
        PPR_model = np.nan  # HanVMe cases where spikes are not found
...
for region in Regions:
    ...
    result_control = differential_evolution(objective_function, bounds, strategy='best1bin', maxiter=100)  
    #result_control = minimize(objective_function, initial_params, method='L-BFGS-B', bounds=bounds)
    ...
    bounds_scaling = [(0.01, 1.01), (0.8, 1.2), (0.8, 1.2)]  # Limits for scaling factors
    if observed_PPR < 1.05 and observed_PPR_quinpirole > 1.05:
    ...
    result_quinpirole = differential_evolution(objective_function_quinpirole_p, bounds_scaling, strategy='best1bin', maxiter=100) 
    ...
    if (PPR_model_quinpirole[0] > 1 and observed_PPR_quinpirole < 1) or (PPR_model_quinpirole[0] < 1 and observed_PPR_quinpirole > 1) or abs(PPR_model_quinpirole-observed_PPR_quinpirole)>0.4:
        print(f"Excluding cell due to mismatch in PPR behavior.")
    ...
    fitted_params_quinpirole_dict = {
        ...
        "Kc": control_params[1],
        'calcium_influx': control_params[6],
        'tau_f': control_params[2]*scaling_params_quinpirole[2],
        'tau_d': control_params[3]*scaling_params_quinpirole[1],
        'tau_rec': control_params[4],
        'tau_Ca': control_params[5],
...
#control_params_dict = fitted_params_control_list
#fitted_params_quinpirole_dict = fitted_params_quinpirole_list
...
observed_PPR_control = ppr_values.mean()
observed_PPR_quinpirole = ppr_values_quinpirole.mean()
...
axes[0, 0].set_title(f'Cells - Control Condition')
...
axes[0, 1].set_title(f'Cell - Quinpirole Condition')
...
plt.tight_layout()
plt.savefig(f"/Users/reva/Documents/Python/SPSynapse/results/plots/trace/VM_mean.pdf")
...
for region in Regions:
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(f"Stat_{region}_mean.pdf")
...
# Create half-violin plot for Baseline
        ...
        try:
            ...
            # HanVMe case when all pairs have zero difference
    ...
    plt.tight_layout()
    plt.savefig(f"Stat_{region}_pairwise_mean.pdf")
